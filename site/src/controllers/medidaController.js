...
function buscarUltimasMedidasEUA(req, res) { 
    ...
    medidaModel.buscarUltimasMedidasEUA(idFuncionario).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
...
function buscarMedidasEmTempoRealEUA(req, res) {

    const idFuncionario = req.body.funcionarioServer;
  
    console.log(`Recuperando medidas em tempo real`);

    medidaModel.buscarMedidasEmTempoRealEUA(idFuncionario).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
